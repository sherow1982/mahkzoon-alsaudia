...
# -*- coding: utf-8 -*-
"""
سكريبت إصلاح ملفات المنتجات المفقودة
يعمل محليًا - ينشئ الملفات مباشرة في مجلد products
"""
...
def create_product_html(product_id, title, image, price, sale_price):
  ...
  <div class="product-grid">
    ...
    <div class="summary">
      ...
      <div class="price-box">
        ...
        <span class="badge">-{int((1 - sale_price/price) * 100)}%</span>
      </div>
...
def main():
    """الدالة الرئيسية"""
    print("="*60)
    print("🔧 سكريبت إصلاح ملفات المنتجات المفقودة")
    print("="*60)
    ...
    # البحث عن المنتجات التي تحتوي على أحرف كبيرة في الأقواس
    pattern = r'\([A-Z]\d+\)'  # يبحث عن (A23), (B15) إلخ
    ...
    for product in products:
        ...
        # التحقق من وجود حرف كبير في الأقواس
    ...
    if not missing_products:
        print("✅ جميع الملفات موجودة! لا يوجد شيء للإصلاح.\n")
        return
    
    print(f"📝 تم العثور على {len(missing_products)} ملف مفقود:\n")
    
    for i, product in enumerate(missing_products, 1):
        print(f"{i}. {product['filename']}")
    ...
    for product in missing_products:
        try:
            # إنشاء محتوى HTML
            html_content = create_product_html(
                product['id'],
                product['title'],
                product['image'],
                product['price'],
            ...
            # حفظ الملف
            with open(product['filepath'], 'w', encoding='utf-8') as f:
                f.write(html_content)
            
            print(f"✅ تم إنشاء: {product['filename']}")
            success_count += 1
            
        except Exception as e:
            print(f"❌ فشل إنشاء {product['filename']}: {str(e)}")
    
    print("\n" + "="*60)
    print(f"✅ تم إنشاء {success_count} ملف بنجاح!")
    print("="*60)
    print("\n💡 الآن يمكنك رفع الملفات إلى GitHub:")
    print("   git add products/")
    print("   git commit -m \"إضافة: صفحات المنتجات المفقودة\"")
    print("   git push")
    print("\n")
